...
Gemini CLI loads this file because [`.gemini/settings.json`](./.gemini/settings.json) sets `context.fileName` to `AGENTS.md`. Leave `CLAUDE.md`, `.claude/CLAUDE.md`, and `CLAUDE.local.md` absent; [docs/AI_ASSISTANTS.md](docs/AI_ASSISTANTS.md) records why a file with one of those names makes Claude Code skip this file.
