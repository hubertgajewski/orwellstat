...
- **Sanitize closing tags before interpolation.** A contributor-controlled value containing the literal `</untrusted-pr-description>` (or any other closing fence tag) would close the fence early and any text after it would land in the structurally trusted region of the prompt. Before substituting `{{DIFF}}`, `{{UNTRACKED}}`, `{{PR_DESC}}`, or `{{BIAS}}`, replace each closing fence tag literal in the value with its entity-encoded form: `</untrusted-diff>` → `&lt;/untrusted-diff&gt;`, `</untrusted-paths>` → `&lt;/untrusted-paths&gt;`, `</untrusted-pr-description>` → `&lt;/untrusted-pr-description&gt;`, `</reviewer-bias>` → `&lt;/reviewer-bias&gt;`. The opening tags do not need escaping — closing the fence early is the only escape that breaks the structural boundary.
