...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- | -------------------- | ------------- | ---------------------------------------------------------- |
| `deep-review-security`          | OWASP Top 10 / CWE Top 25 / OWASP ASVS vulnerability review                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | security-risk trigger                                                                                                                                                                            | H/M/L         | `findings: none`     | HIGH + MEDIUM | `Read, Grep, Glob`                                         |
| `deep-review-project-checklist` | orwellstat-specific Playwright / POM / fixture / tag / CI-workflow conventions                                                                                                                                                                                                                                                                                                                                                                                                                                                      | project-checklist trigger                                                                                                                                                                        | pass/fail/N/A | `Failures: none.`    | fail          | `Read, Grep, Glob`                                         |
...
| `deep-review-docs`              | README / docs / CLAUDE.md / skill-file consistency against the project's documented split rules                                                                                                                                                                                                                                                                                                                                                                                                                                     | docs trigger                                                                                                                                                                                     | pass/fail/N/A | `Failures: none.`    | fail          | `Read, Grep, Glob`                                         |
...
Per-agent diff selection:

- `deep-review-security` — conservative full `DIFF`; security review may need cross-file data-flow and risk classification outside extension-specific hunks.
- `deep-review-simplification` — conservative full `DIFF`; reuse and duplication findings often span unrelated-looking files.
- `deep-review-code` — conservative full `DIFF`; general functionality and test-coverage review is intentionally broad.
- `deep-review-architecture` — conservative full `DIFF`; coupling and dependency-direction issues are cross-cutting by design.
- `deep-review-project-checklist` — relevant hunks only: paths matching the project-checklist trigger surfaces (`playwright/typescript/**`, `bruno/**`, workflows, `action.yml`, or `action.yaml`).
- `deep-review-docs` — relevant hunks only: paths matching the docs trigger surfaces, new-file hunks, and hunks whose added lines include one of the docs trigger's environment or repository variable markers.
- `deep-review-typescript` — relevant hunks only: `*.ts` and `*.tsx` paths.
- `deep-review-python` — relevant hunks only: `*.py` paths.
- `deep-review-ci` — relevant hunks only: `.github/workflows/**.yml`, `.github/workflows/**.yaml`, `action.yml`, and `action.yaml`.
- `deep-review-qa` — relevant hunks only: `playwright/typescript/tests/**/*.spec.ts`, `playwright/typescript/**/*.setup.ts`, `bruno/**/*.bru`, `playwright/typescript/fixtures/**`, and `playwright/typescript/test-data/**`.
- `deep-review-unit-test` — relevant hunks only: `scripts/**/*.py`, `mcp/**/*.ts`, `playwright/typescript/utils/**/*.ts`, `playwright/typescript/scripts/**/*.ts`, and adjacent test files owned by that unit-test surface.

When a non-broad specialist is dispatched but its selector produces no inline hunk (for example, because the only relevant item is an untracked path), pass an empty `<untrusted-diff>` block or omit it under the normal empty-block rule, keep the complete `CHANGED_FILES` and `UNTRACKED` blocks, and rely on `Read` for the file contents. Do not silently fall back to the full diff for scoped specialists; that would erase the token-saving contract. Only the four broad agents above intentionally receive the full diff.
