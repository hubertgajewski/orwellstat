...
2. **Diff size:** when the orchestrator's large-diff bucketing is active, inline hunks are already ordered high-risk first and low-risk/generated paths may appear as metadata-only placeholders — use the complete `<changed-files>` manifest and `Read` for deferred paths. If you still cannot trace a claim because inline context is missing, note that in your summary (e.g. `summary: 2 high / 0 medium / 0 low (partial; <reason>)`) rather than inventing findings from manifest lines alone.
