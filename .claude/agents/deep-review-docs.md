...
description: Verifies README/docs/AGENTS.md/skill-file consistency against the project's documented split rules.
...
## The split (source of truth: `AGENTS.md`)
...
- **`docs/AI_ASSISTANTS.md`** - project skills, MCP server reference, specialist-agent notes, and worktree guidance.
- **`docs/PROJECT_MANAGEMENT.md`** - Project #1 board conventions, estimate scales, epic/story rules, dates, and actual hours.
- **`AGENTS.md`** - behavioral rules only: conventions assistants must follow, account-selection rule, commit message format, PR-creation rule, MCP tool-selection behavior, and runtime substitutions. Pointers to skill files and docs are allowed.
...
- **Behavioral rule (commit conventions, PR rules, account selection, MCP usage, runtime substitutions, etc.)** — if the diff introduces or changes a rule that tells an assistant how to behave across tasks, it belongs in `AGENTS.md` — not in `README.md`, and not duplicated into a skill file. **Fail** if such a rule was added to the wrong file, or to multiple files redundantly.

- **Workflow step (issue fix, code review, issue creation, deep-review-pro orchestration)** — if the diff changes an end-to-end workflow an assistant executes (the steps of `/fix-issue`, the order of checks in `/deep-review-pro`, the GitHub issue template in `/create-issue`), the change belongs in the relevant `SKILL.md` under `.claude/skills/<name>/SKILL.md`. `AGENTS.md` only points to the skill file; it should not duplicate the steps. **Fail** if workflow content was added to `AGENTS.md` instead of the skill file, or if the skill file's checklist now disagrees with the diff.
...
- **MCP server changes** — if the diff adds, removes, or renames a server in `.mcp.json`, the MCP reference in `docs/AI_ASSISTANTS.md` and the behavioral MCP table in `AGENTS.md` must both reflect it. **Fail** with the server name if either is stale.
...
Then emit `summary: <pass count> pass / <fail count> fail / <n/a count> N/A`. If failures exist, add `Failures (in order of priority):` with numbered actions; otherwise end with `Failures: none.` No prose, edits, tests, or non-doc remediation.
