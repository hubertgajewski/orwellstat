...
- **style** — PEP 8 violations the diff introduces: line length over the project limit, mixed tabs/spaces, naming convention drift (snake_case for functions/variables, PascalCase for classes, UPPER_CASE for constants), import ordering and grouping (stdlib / third-party / first-party, with blank lines between groups), whitespace around operators, trailing whitespace, missing or extra blank lines around top-level definitions. Common mappings: `[PEP-8]`. Include the equivalent ruff code in parentheses (`E501`, `E701`, `E711`, `W291`, `I001`, `N802`, `N806`).
- **idiom** — non-Pythonic constructs the diff introduces, anchored in PEP 20: index-based loops where `enumerate` / direct iteration would read; manual list-building where a comprehension would; mutable default arguments; `type(x) == X` instead of `isinstance(x, X)`; `len(x) == 0` instead of `not x` (and vice versa where ambiguity matters); `except:` without an exception class; needlessly nested control flow ("flat is better than nested"); explicit re-raising patterns that swallow context (`raise X from None` only when the chain is actively misleading). Common mappings: `[PEP-20]`. Include the equivalent ruff code in parentheses where one exists (`SIM*`, `B*`, `UP*`, `RUF*`).
...
- **bug-risk** — diff-introduced constructs that ruff's `F` family (and select `B` lints) would flag as latent bugs rather than style: unused imports / unused local variables that are not deliberate re-exports, undefined names, comparisons to `None` with `==`, mutable default arguments (when the call site mutates), bare `except:` (when an exception class is the right move), `assert` used for non-test runtime checks. Common mappings: `[PEP-8]` (when the shape is style-driven), `[PEP-20]` (when the shape is idiom-driven). Include the equivalent ruff code in parentheses (`F401`, `F811`, `F821`, `F841`, `B006`, `B011`, `E711`, `E712`).
