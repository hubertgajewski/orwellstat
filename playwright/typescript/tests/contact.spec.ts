...
test('contact page - headings and statsbar content', async ({ page }) => {
  ...
  await expect(statsbar.getByText(ContactPage.loggedInAs, { exact: false })).toBeVisible();
  await expect(
    statsbar.getByRole('button', { name: ContactPage.logoutButton, exact: true })
  ).toBeVisible();

  // Contact section – text and external link
  await expect(statsbar.getByText(ContactPage.emailIntro, { exact: false })).toBeVisible();
