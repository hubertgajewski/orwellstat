...
| `tests/zone-stats.spec.ts`       | `/zone/stats/`   | Empty-account no-chart/no-table state; SVG chart structural analysis, user-statistics table, and parameterized `Parametr` loop validating chart labels/percentages against data-table rows for all 12 dimensions, while exempting only the IP/hostname pair from cross-parameter distinctness. |
...
| Spec                                       | Scope                             | Coverage                                                                                                                                                                       |
| ------------------------------------------ | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `utils/svg-chart-distinctness.util.test.ts` | Statistics parameter distinctness | Permits IP/hostname fallback equality in either order, detects non-exempt duplicate labels, and reports both colliding parameter values.                                       |
