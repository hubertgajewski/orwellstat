...
| `tests/zone-stats.spec.ts`       | `/zone/stats/`   | Empty-account no-chart/no-table state; SVG chart structural analysis, user-statistics table, and parameterized `Parametr` loop comparing chart labels/percentages against data-table rows for all 12 dimensions while allowing legitimate IP/hostname fallback equality. |
...
| Spec                                       | Scope                             | Coverage                                                                                                                                                                       |
| ------------------------------------------ | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `utils/svg-chart-distinctness.util.test.ts` | Statistics parameter distinctness | Permits IP/hostname fallback equality in either comparison order while retaining distinctness for every other parameter pair.                                                  |
