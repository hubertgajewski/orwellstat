...
- `tests/` — Playwright test specs (`.spec.ts`)
  ...
  - `zone-stats.spec.ts` — Authenticated `/zone/stats/` page: SVG chart load (per-user `chart.php`) plus one parameterised test per "Pokaż statystyki" Parametr option asserting the form round-trip, chart reload, and a non-empty data table; tagged `@regression`
...
- `utils/svg-chart.util.ts` — `navigateAndWaitForSvgChart(page, pageUrl, svgChartUrlFragment, preAuthUrl?)` shared helper: navigates to a stats page and waits for its SVG chart sub-resource (`chart.php` on `/zone/stats/`, `chart_all.php` on `/statistics/`) to load with the right `content-type`, then asserts `object[type="image/svg+xml"]` is visible. Optional `preAuthUrl` pre-navigates to prime Firefox's Basic Auth credential cache on staging. Used by `tests/statistics.spec.ts` and `tests/zone-stats.spec.ts` so neither file duplicates the chart-load wait
