...
- `utils/__tests__/diagnosis.util.test.ts` — `node:test` unit suite for `redactSensitive`, exercising cookie / bearer / email / mixed / no-match / char-budget / XHTML-structure / multi-line cases; runs via `npm run test:unit` locally and on every PR through `playwright-typescript-lint.yml`
