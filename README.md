...
| `/deep-review-lite` | `/deep-review-lite`           | Works through every item on the code review checklist from `.claude/skills/deep-review-lite/SKILL.md`, applies general diff checks and CI workflow checks, and explicitly states a finding (pass / fail / N/A) for each item                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `/deep-review-pro`  | `/deep-review-pro [arg]`      | Multi-agent code review orchestrator — dispatches every project-scoped specialist agent under `.claude/agents/` in parallel against a scope resolved from `arg`: empty (local diff, **US1**), a PR number with optional bias (`213` or `213 focus on race conditions`, **US2**), a git ref or range (`HEAD~3..HEAD`, **US3a**), a file or directory path (`./scripts/self-healing.py`, **US3b**), or any other freeform instruction (**US3c**); surfaces their findings together. The current roster (security / project-checklist / simplification / code / architecture / typescript / python / docs / ci / qa / unit-test) is documented in `.claude/skills/deep-review-pro/SKILL.md` |
...
.env.example                # template: ORWELLSTAT_USER, ORWELLSTAT_PASSWORD, ORWELLSTAT_USER_EMPTY, ORWELLSTAT_PASSWORD_EMPTY, ORWELLSTAT_EMAIL, ENV, BASIC_AUTH_USER, BASIC_AUTH_PASSWORD, ORWELLSTAT_DRAIN_TOKEN, ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENROUTER_API_KEY
.vars                       # CI repository variables (git-ignored); see .vars.example
.vars.example               # template: AI_REVIEW, PLAYWRIGHT_TYPESCRIPT, BRUNO, QUALITY_METRICS, AI_DIAGNOSIS, AI_PROVIDER, AI_MODEL_FAST, AI_MODEL_STRONG, ANTHROPIC_BASE_URL, ANTHROPIC_DEFAULT_HAIKU_MODEL, ANTHROPIC_DEFAULT_SONNET_MODEL, ANTHROPIC_DEFAULT_OPUS_MODEL, SELF_HEALING
.node-version               # CI Node release line consumed by actions/setup-node@v6 with check-latest
...
.claude/
  skills/                   # project-scoped slash commands (fix-issue, create-issue, deep-review-lite, deep-review-pro, generate-stubs, generate-test)
  agents/                   # project-scoped sub-agents dispatched by skills (current roster lives in .claude/skills/deep-review-pro/SKILL.md)
.agents/
...
.github/actions/            # local composite actions used by workflows (e.g. Playwright browser setup)
...
| ----------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Node.js](https://nodejs.org/) v26.x                              | Playwright, Bruno, and MCP test tooling          | [nodejs.org](https://nodejs.org/)                                                                                                                                              |
...
| [actionlint](https://github.com/rhysd/actionlint)                 | `/deep-review-pro` GitHub Actions reviewer agent | macOS: `brew install actionlint shellcheck` (requires [Homebrew](https://brew.sh)); other platforms: [rhysd/actionlint releases](https://github.com/rhysd/actionlint/releases) |

Node.js includes `npm` — no separate installation needed. CI workflows use `actions/setup-node@v6` with `node-version-file: .node-version` (currently `26`) and `check-latest: true` so they track the latest available 26.x release. Docker, `act`, and `actionlint` are optional: Docker and `act` are only needed for local CI testing; `actionlint` is only needed when running the `/deep-review-pro` GitHub Actions reviewer agent locally.
...
| `ANTHROPIC_DEFAULT_OPUS_MODEL`   | Opus-tier model slug for PR reviewer; required when `ANTHROPIC_BASE_URL` is set                                                                                                                                                                                                                                                                                                                                                                 | PR events via `claude-code-review.yml`                                                                                                                              |
...
- `tests/` — Playwright test specs (`.spec.ts`)
  ...
  - `visual.spec.ts` — Full-page visual regression snapshots for home (default and all style variants), about system, contact, public statistics, register, password reset, previously added, information, stats, hits, scripts, and admin pages using `toHaveScreenshot()` with `maxDiffPixelRatio: 0.01`; every screenshot that renders the shared logged-in block masks the logged-in username; home and previously-added pages also mask `#statsbar` lists (dynamic new-browser/OS list items) via `getByRole('list')`; public statistics and authenticated stats pages mask `getByRole('table')` (live data) and `object[type="image/svg+xml"]` (dynamic SVG chart), remove all but the first 5 rows from the statistics table via `page.evaluate()` to keep the footer at a stable position regardless of how many browser/OS rows live data contains (CSS height/overflow tricks are ineffective here: Playwright's `fullPage` screenshot and mask both use the element's full bounding box, not the clipped visual; physically removing rows is the only reliable fix), wait for `<object>` to be visible before screenshotting to stabilise layout, and disable animations; information preserves headings/labels while normalizing and masking only live summary/ranking values, register also masks the rendered IP, hits trims and masks the live results table, scripts masks snippet textareas, and admin masks account identifiers/email/block-IP while leaving stable password-help copy visible; baselines stored in `tests/visual.spec.ts-snapshots/` with per-platform suffixes (`-darwin`, `-linux`); tagged `@regression`
...
- `scripts/patch-playwright-yauzl-node26.mjs` — Post-install patch for `playwright-core@1.59.1` that applies Playwright upstream PR #40747's vendored `yauzl` stream-destroy fix. Without it, `npx playwright install` hangs after a browser archive reaches 100% under Node 26. Unit-tested in `scripts/patch-playwright-yauzl-node26.test.ts`. Remove this script and the `postinstall` hook once a stable Playwright release containing #40747 is adopted.
...
**Verify Coverage Matrix:** `.github/workflows/verify-coverage-matrix.yml` — runs on PRs and pushes that touch `.node-version`, `coverage-matrix.json`, anything under `playwright/typescript/tests/`, or the verifier script itself. Executes `npm run verify:matrix`, which fails non-zero with a per-cell diff when the matrix's claims do not match the active-test inventory. The two failure modes are listed in the script output: **false-positive** (matrix claims `true` but no covering test exists — typically a forgotten matrix update after a test was deleted or a stale boolean) and **false-negative** (an active test covers a cell the matrix has as `false` — typically a forgotten flip after adding a new test).

**MCP Tests:** `.github/workflows/mcp-tests.yml` — runs on PRs and pushes that touch `mcp/**`, `.node-version`, or the workflow file itself; the single `test` job (`timeout-minutes: 10`) uses `node-version-file: .node-version` with `check-latest: true`, builds `mcp/shared`, then fans out across `coverage-matrix` and `quality-metrics` to run `npm ci`, `npm audit --audit-level=high`, and `npm test` for each package. The workflow uses the standard per-ref concurrency group with `cancel-in-progress: true` so superseded PR pushes do not keep stale MCP test jobs running.
...
Five MCP servers are declared in `.mcp.json` and loaded automatically by any MCP-compatible AI assistant opened from the repo root. All five are loaded automatically — no local setup needed beyond having Node.js 26.x and Docker installed (and a one-off `npm install && npm run build` in `mcp/shared/` followed by the same in `mcp/quality-metrics/` and `mcp/coverage-matrix/` for the local servers; both consumers depend on the shared package via a `file:../shared` path, so it must be built first).
...
`playwright-report-mcp@3.1.3` declares `node >=22`; this is lower than the repository's Node.js 26.x tooling baseline, so use Node.js 26.x for local Playwright, Bruno, and MCP workflows.
