...
- `tests/` — Playwright test specs (`.spec.ts`)
  ...
  - `visual.spec.ts` — Full-page visual regression snapshots for home (default and all style variants), about system, contact, public statistics, register, password reset, previously added, information, stats, hits, scripts, and admin pages using `toHaveScreenshot()` with `maxDiffPixelRatio: 0.01`; home and previously-added pages mask `#statsbar` lists (dynamic new-browser/OS list items) via `getByRole('list')`; public statistics and authenticated stats pages mask `getByRole('table')` (live data) and `object[type="image/svg+xml"]` (dynamic SVG chart), remove all but the first 5 rows from the statistics table via `page.evaluate()` to keep the footer at a stable position regardless of how many browser/OS rows live data contains (CSS height/overflow tricks are ineffective here: Playwright's `fullPage` screenshot and mask both use the element's full bounding box, not the clipped visual; physically removing rows is the only reliable fix), wait for `<object>` to be visible before screenshotting to stabilise layout, and disable animations; information normalizes and masks live summary/ranking prose, hits trims and masks the live results table, and scripts/admin mask live account identifiers or snippet values while still capturing page structure; baselines stored in `tests/visual.spec.ts-snapshots/` with per-platform suffixes (`-darwin`, `-linux`); tagged `@regression`
