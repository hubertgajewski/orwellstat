...
| `/deep-review-next` | `/deep-review-next [arg]`     | Multi-agent code review orchestrator — dispatches every project-scoped specialist agent under `.claude/agents/` (currently `deep-review-security` and `deep-review-project-checklist`) in parallel against a scope resolved from `arg`: empty (local diff), a PR number (`213`), a freeform instruction, or a combination (`213 focus on race conditions`); surfaces their findings together. Coexists with `/deep-review` until promoted by atomic dir rename (#435) |
