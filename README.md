...
| `@regression`           | category | Deep checks: table content, SVG analysis, external link hrefs, accessibility, visual | `home.spec.ts`, `about-system.spec.ts`, `contact.spec.ts`, `statistics.spec.ts`, `svg-chart-util.spec.ts`, `accessibility.spec.ts`, `validation.spec.ts`, `visual.spec.ts`, `network-mocking.spec.ts`, `register.spec.ts`, `password-reset.spec.ts`, `zone-information.spec.ts`, `zone-stats.spec.ts`, `zone-hits.spec.ts`, `zone-scripts.spec.ts`, `zone-admin.spec.ts`, `forms.spec.ts` |
...
- `tests/` — Playwright test specs (`.spec.ts`)
  ...
  - `svg-chart-util.spec.ts` — Shared SVG chart utility regression: verifies `dataTableTopRowsFromXhtml` reads table rows from a captured XHTML snapshot, trims whitespace, preserves non-ASCII labels, and returns `[]` when the snapshot has no table; tagged `@regression`
