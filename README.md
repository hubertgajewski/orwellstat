...
**MCP Tests:** `.github/workflows/mcp-tests.yml` — runs on PRs and pushes that touch `mcp/**` or the workflow file itself; the single `test` job (`timeout-minutes: 10`) uses Node 26 with `check-latest: true`, builds `mcp/shared`, then fans out across `coverage-matrix` and `quality-metrics` to run `npm ci`, `npm audit --audit-level=high`, and `npm test` for each package. The workflow uses the standard per-ref concurrency group with `cancel-in-progress: true` so superseded PR pushes do not keep stale MCP test jobs running.
